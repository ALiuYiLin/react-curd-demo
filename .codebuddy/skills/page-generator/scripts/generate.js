 ...
 * 
 * 用法: node .codebuddy/skills/page-generator/scripts/generate.js <pageName> [entityLabel]
 * 示例: node .codebuddy/skills/page-generator/scripts/generate.js user 用户
 *       node .codebuddy/skills/page-generator/scripts/generate.js product 产品
 * 
 * 读取 config/page/<pageName>.csv
 * 使用 .codebuddy/skills/page-generator/templates/ 下的 hbs 模板
 * 输出到 .codebuddy/skills/page-generator/output/<pageName>/
 */
...
const CONFIG_DIR = path.join(ROOT_DIR, 'config/page');
...
function generatePage(pageName, entityLabel) {
  const csvPath = path.join(CONFIG_DIR, `${pageName}.csv`);
...
if (!pageName) {
  console.log('用法: node generate.js <pageName> [entityLabel]');
  console.log('示例: node generate.js user 用户');
  console.log('      node generate.js product 产品');
  process.exit(1);
}
